...
# Program Timing:
################################################################
MINIMUPDATETIME                  = 1./20.                   # 20 fps
KEEPALIVEINTERVAL                = 10                       # Every 10 secs
...
class gearVRC:
    ...
    def __init__(self, device_name=None, logger=None, VRMode=False) -> None:
        ...
        self._lastTimeFPS        = time.perf_counter()
        self.deltaTime           = 0.
    ...
    def handle_sensorData(self, characteristic: BleakGATTCharacteristic, data: bytearray):
        ...
        self.deltaTime = self._currentTime - self._lastTime
        self._lastTime = copy(self._currentTime)
        # self.logger.log(logging.DEBUG, 'Update rate: {}'.format(self.deltaTime))
        if self.deltaTime > self._updateTime: 
            self.logger.log(logging.INFO, 'Update rate slow')

        if self._currentTime - self._lastTimeFPS >= 1:
            self.fps = self._updatecounts
            self._lastTimeFPS = self._currentTime
            self._updatecounts = 0
            # self.logger.log(logging.DEBUG, 'FPS: {}'.format(self.fps))
        ...
        if (len(data) >= 60):
            ...
            self.acc = calibrate(self.acc, self.acc_offset, self.acc_scale, self.acc_crosscorr)
            self.mag = calibrate(self.mag, self.mag_offset, self.mag_scale, self.mag_crosscorr)
            self.gyr = calibrate(self.gyr, self.gyr_offset, self.gyr_scale, None)
    ...
    async def stop_sensor(self):
        if self._client is not None:
            ...
            self.logger.log(logging.ERROR, 'Could not read stop: disconnected')
    ...
    async def keep_alive(self):   
        while not self.finish_up.is_set():
            if self._client is not None and self.connected.is_set() and self.sensorStarted.is_set() and (not self.lost_connection.is_set()):
                ...
                self.logger.log(logging.INFO,'Cound not send Keep alive')                
                sleepTime=1
    ...
    async def report(self):
        ...
        while not self.finish_up.is_set():
            ...
            self.logger.log(logging.INFO, 'gearVR Controller: Temp {:>4.1f}, Bat {:>3d}, fps: {:>3d} dT:{:>5.1f}'.format(self.temperature, self.battery_level, self.fps, self.deltaTime*1000.))
            self.logger.log(logging.INFO, '--------------------------------------------------------')
            ...
            self.logger.log(logging.INFO, 'Wheel: {:>2d}:{:>3d} T{} B{} L{} R{} C{} R:{}'.format(
                                                ...
                                                'Y' if self.center else 'N', 
                                                (' C' if self.clockwise else 'CC') if self.isRotating else '  '))
...
async def main(args: argparse.Namespace):
    ...
    if os.name == 'posix':
        ...
        loop.add_signal_handler(signal.SIGINT,  lambda: asyncio.create_task(controller.handle_termination([keepalive_task, reporting_task, connection_task])) ) # control-c
        loop.add_signal_handler(signal.SIGTERM, lambda: asyncio.create_task(controller.handle_termination([keepalive_task, reporting_task, connection_task])) ) # kill

    # These tasks will not terminate 
    if args.report:
        await asyncio.wait([connection_task, keepalive_task, reporting_task], timeout=float('inf'))
    else:
        await asyncio.wait([connection_task, keepalive_task], timeout=float('inf'))
