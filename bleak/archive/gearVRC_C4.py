...
class gearVRC:
    ...
    async def update_report(self):
        ...
        while not self.finish_up.is_set():
            ...
            # Display the Data
            str  = '--------------------------------------------------------\n'
            str += 'gearVR Controller: Temp {:>4.1f}, Bat {:>3d}, fps: {:>3d} dT:{:>5.1f}\n'.format(self.temperature, self.battery_level, self.data_fps, self.data_deltaTime*1000.)
            str +=  '--------------------------------------------------------\n'
            ...
            # self.logger.log(logging.INFO, 'Report  {:>10.6f}, fps {:>3d}'.format(self.report_deltaTime, self.report_fps))
