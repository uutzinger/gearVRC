...
class gearVRC:
    ...
    async def find_characteristics(self):
        ...
        ########################
        self.generic_access_profile_service = next(
            s for s in self._client.services
            if s.uuid == '00001800-0000-1000-8000-00805f9b34fb')
        
        # read
        self._device_name_characteristic = next(
            c for c in self.generic_access_profile_service.characteristics
            if c.uuid == '00002a00-0000-1000-8000-00805f9b34fb')

        # Device Information
        ####################
        self._device_information_service = next(
            s for s in self._client.services
            if s.uuid == '0000180a-0000-1000-8000-00805f9b34fb')
        # read
        self.manufacturer_name_characteristic = next(
            c for c in self._device_information_service.characteristics
        ...
        # read & write
        self.controller_command_characteristics = next(
            c for c in self.controller_data_service.characteristics
    ...
    async def read_deviceInformation(self):
        ...
        # Device Name
        try: 
            value = await self._client.read_gatt_char(self._device_name_characteristic)
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(main(args), debug=True if use_VisualCode else False)
